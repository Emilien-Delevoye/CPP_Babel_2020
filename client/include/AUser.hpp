...
class AUser : public CustomButton {
    Q_OBJECT
    public:
    explicit AUser(QWidget *parent = nullptr, const std::string &login = "login", const std::string &ip = "ip", short port = -1, int id = -1) : CustomButton(parent, QString(login.c_str())), _login(login), _ip(ip), _port(port), _id(id) {};
